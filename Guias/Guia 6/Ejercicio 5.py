...
w = np.logspace(-2, 2, 1000) # 0.01 a 100 rad/seg 
w, h = sig.freqs(a, b)
...
fs = 10 # <-- PROBAR distintos valores: 0.5, 1, 3
num, den = sig.bilinear(b, a, fs)

# Respuesta en frecuencia digital (theta en [0, pi] rad/muestra) 
wz, hz = sig.freqz(num, den, worN = 1000)
plt.figure()
...
zz, pz, kz = sig.tf2zpk(num, den)
print("ceros (z): radio", np.abs(zz), " angulo[rad]", np.angle(zz))
print("polos (z): radio", np.abs(pz), " angulo[rad]", np.angle(pz))
