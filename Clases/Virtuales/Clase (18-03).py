...
plt.plot(tt, xxn, label ='Senoidal con Ruido y potencia normalizada')
plt.grid()
plt.legend()
plt.show()
#%%
...
n0 = 300 # muestras
dd = np.zeros(n0 + 1) # delta dirac --> creo un vector de ceros, y le digo que en un solo valor valga 1
dd[n0] = 1 # Levanto el vector en n0 para que ahi valga 1

plt.figure()
plt.title(f'Delta en n0 = {n0}')
plt.plot(dd)
plt.grid()
plt.show()
...
plt.figure()
plt.title(f'Senoidal corrida n0 = {n0} muestras')
plt.plot(yy)
plt.grid()
plt.show()

#%% CONVOLUCION
# El tamaño es la longitud de una señal + la longitud de la otra señal - 1
...
# La autocorrelacion siempre es par, tiene simetria par
# La secuencia no es estrictamente incorrelada --> no es blanco --> aparece ese "piso" de ruido
# La secuencia es idelmente incorrelada, por eso la delta no queda donde creemos que deberia
# La transformada de Furier no va a ser una constante, porque no es una delta, es parecida a una delta 
# --> me va a dar algo parecido a la transformada de una delta, el espectro me va a dar parecido a una constante
